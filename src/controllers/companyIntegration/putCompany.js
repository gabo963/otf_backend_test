...
const putCompany = async (newCompanyInfo) => {
    ...
    const { name, dimension, createdate, location_id, creation_date, location_type } = newContactInfo;

    const response = await hubspotClient.crm.companies.basicApi.update(newCompanyInfo.id, {
        properties: { name, dimension, createdate, location_id, creation_date, location_type },
    });
